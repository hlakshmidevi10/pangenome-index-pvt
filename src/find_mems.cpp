...
void dump_mem_info(const MEM& mem, const int read_id, TagArray& tag_array, FastLocate& r_index,
    vector<size_t> &seq_id_counter, double& total_rindex_locate_time, double& total_query_decoded_runs_time,
    ...
    total_rindex_locate_time += locate_duration.count();
#endif

#if TIME
    auto decoded_runs_start = chrono::high_resolution_clock::now();
#endif
...
#if TIME
    auto decoded_runs_end = chrono::high_resolution_clock::now();
    std::chrono::duration<double> decoded_runs_duration = decoded_runs_end - decoded_runs_start;
    total_query_decoded_runs_time += decoded_runs_duration.count();
#endif
...
int main(int argc, char **argv) {
    if (argc < 6) {
        std::cerr << "Usage: " << argv[0] << " <r_index_file> <tag_array_index> <reads_file> <mem_length> <min_occ> [output_file] [--debug-stats]" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (argc > 6 && std::string(argv[6]) != "--debug-stats") {
    ...
    double total_rindex_locate_time = 0.0;
    double total_query_decoded_runs_time = 0.0;
#if TIME
    auto time1 = chrono::high_resolution_clock::now();
    double total_mem_query_time = 0.0;
    double total_tag_time = 0.0;
    double total_dump_mem_info_time = 0.0;
    double total_sort_output_time = 0.0;
#endif
...
//     exit(0);
    ...
    while (std::getline(reads, read)) {
        ...
        total_mem_query_time += duration3.count();
#endif
...
//            std::cout << "BWT interval start: " << mem.bi_interval.forward << ", size: " << mem.bi_interval.size << std::endl;
...
#if TIME
            auto dump_mem_start = chrono::high_resolution_clock::now();
#endif

            dump_mem_info(mem, i, tag_array, r_index, seq_id_counter, total_rindex_locate_time,
                total_query_decoded_runs_time, output_file.is_open() ? &output_file : nullptr, debug_stats);

#if TIME
            auto dump_mem_end = chrono::high_resolution_clock::now();
            std::chrono::duration<double> dump_duration = dump_mem_end - dump_mem_start;
            total_dump_mem_info_time += dump_duration.count();
#endif
    ...
    std::chrono::duration<double> total_reads_processing_time = reads_processing_end - reads_processing_start;
#endif
    ...
    if (argc > 6) {
        ...
        total_sort_output_time = sort_duration.count();
#endif
    }

#if TIME
    std::cout << "\n=== PERFORMANCE PROFILING RESULTS ===" << std::endl;
    std::cout << "Total time for querying all MEMs: " << total_mem_query_time << " seconds" << std::endl;
    std::cout << "Total time for r_index.locate() calls: " << total_rindex_locate_time << " seconds" << std::endl;
    std::cout << "Total time for query_compressed_decoded_runs: " << total_query_decoded_runs_time << " seconds" << std::endl;
    std::cout << "Total time for dump_mem_info (excluding locate & decoded_runs): "
              << (total_dump_mem_info_time - total_rindex_locate_time - total_query_decoded_runs_time) << " seconds" << std::endl;
    std::cout << "Total time for dump_mem_info (including all): " << total_dump_mem_info_time << " seconds" << std::endl;
    std::cout << "Total time for tag queries (query_compressed): " << total_tag_time << " seconds" << std::endl;
    std::cout << "Total time for sorting/output file generation: " << total_sort_output_time << " seconds" << std::endl;
    std::cout << "Total time for processing all reads: " << total_reads_processing_time.count() << " seconds" << std::endl;
    std::cout << "=====================================" << std::endl;
#endif
}
